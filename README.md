...
### 7. Redis 캐싱 적용 — KOSIS 지역 평균 전세가율

KOSIS 통계는 **지역코드 + 주택유형** 조합이 같으면 항상 동일한 값을 반환하는 월 1회 갱신 데이터임에도,
리팩토링 전에는 매 요청마다 외부 API를 새로 호출하고 있어 불필요한 네트워크 지연이 발생하고 있었다.

- `@Cacheable`을 적용해 첫 요청 결과를 Redis에 저장, 이후 동일 조건 요청은 Redis에서 즉시 반환
- **캐시 키:** `buildingType + "_" + 지역코드` (예: `아파트_a7020203`)
- **빈 결과는 캐싱 제외(`unless`)** — 구 단위 조회 실패 시 시·도 단위로 fallback하는 로직이 영구 차단되는 것을 방지
- **TTL: 1일** — 월별 갱신 데이터지만, KOSIS 측 수정 배포 등 예외 상황을 고려해 보수적으로 설정

**성능 테스트 결과** (서울 송파구 / 아파트, 동일 조건 2회 연속 호출)

| | 1차 호출 | 2차 호출 |
|--|--|--|
| 캐싱 전 | 5,723ms | 156ms (API 재호출) |
| 캐싱 후 | 785ms | **7ms** (Redis 히트) |

2차 호출 기준 약 **22배** 응답속도 개선
